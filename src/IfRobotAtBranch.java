...
public class IfRobotAtBranch extends Node{
	 ...
	 */
	public IfRobotAtBranch(final Node robot, final Node child1, final Node child2) {
	 ...
	 * @param child2 The second conditionally evaluated child node.
	 */
	public IfRobotAtBranch(final TrainingAndTestSet trainingAndTestSet, final Node child1, final Node child2) {
		super(child1, child2);
	 ...
	 * is evaluated. An exception will be thrown if this argument is null.
	 */
	public IfRobotAtBranch(final TrainingAndTestSet trainingAndTestSet) {
	...
	public Void evaluate() {
		...
		this.robotsAndGraph = this.robotsAndGraphList.list[this.robotsAndGraphList.currExample];
		//System.out.println("IF ROBOT AT BRANCH");
		...
		boolean atBranch = false;
		
		ArrayList<GraphNode> branches = this.robot.getBranches(this.robot).get(0);
		
		// Must be at branch that it set its path to
		if(branches.contains(this.robot.currNode) && this.robot.path.size() <= 1){
			atBranch = true;
			this.robot.goingToBranch = false;
			// Reset robot's path towards destination
		...
		if (atBranch) {
			...
			//System.out.println("NOT AT BRANCH");
			child2.evaluate();
		}
