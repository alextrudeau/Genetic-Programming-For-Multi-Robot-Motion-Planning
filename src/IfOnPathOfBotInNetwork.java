...
public class IfOnPathOfBotInNetwork extends Node{
	 ...
	 */
	public IfOnPathOfBotInNetwork(final Node robotsAndGraphList, final Node child1, final Node child2) {
	 ...
	 * @param child2 The second conditionally evaluated child node.
	 */
	public IfOnPathOfBotInNetwork(final TrainingAndTestSet trainingAndTestSet, final Node child1, final Node child2) {
		super(child1, child2);
	 ...
	 * is evaluated. An exception will be thrown if this argument is null.
	 */
	public IfOnPathOfBotInNetwork(final TrainingAndTestSet trainingAndTestSet) {
	...
	public Void evaluate() {
		...
		this.robotsAndGraph = this.robotsAndGraphList.list[this.robotsAndGraphList.currExample];
		//System.out.println("IF ROBOT ON PATH OF ROBOT IN NETWORK");
		...
		*/
		...
		for(int i = 0; i < this.robot.directNetwork.size(); i++){
			...
			if(this.robot.directNetwork.get(i).path.contains(this.robot.currNode)){
				onPath = true;
				break;
			}
		}
		...
		if(onPath){
			//System.out.println("ON PATH OF ROBOT IN NETWORK");
			child1.evaluate();
		}
		else{
			//System.out.println("NOT ON PATH OF ROBOT IN NETWORK");
			child2.evaluate();
		}
	...
	public String getIdentifier() {
		return "IF-ROBOT-ON-PATH-OF-ROBOT-IN-NETWORK";
	}
